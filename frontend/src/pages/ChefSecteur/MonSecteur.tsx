...
const MonSecteur: React.FC = () => {
  ...
  const loadSecteurData = async () => {
    try {
      ...
      if (user?.secteur && typeof user.secteur === 'object') {
        setSecteur(user.secteur);
      }

      // Load services of this secteur - use a simpler approach
      if (user?.secteur && typeof user.secteur === 'object' && user.secteur._id) {
        try {
  ...
  const getChefServiceName = (chefService: string | User | undefined): string => {
    ...
    return `${chefService.firstName || chefService.name || ''} ${chefService.lastName || ''}`.trim();
  };

  // Helper function to get user full name
  const getUserFullName = (user: string | User | undefined): string => {
    if (!user) return 'N/A';
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-gray-900">{getSiteName(secteur.site)}</span>
                      </div>
                      {secteur.description && (
                      ...
                      {secteur.statistics && (
                          <>
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-sm text-gray-900">{secteur.statistics.servicesCount}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <span className="text-sm font-medium text-gray-700">Utilisateurs:</span>
                              <span className="text-sm text-gray-900">{secteur.statistics.usersCount}</span>
                            </div>
                          </>
                      )}
                    </div>
                      ...
                      <div key={service._id || `${service.code}-${idx}`} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex items-center space-x-1 text-sm text-gray-500">
                            ...
                            <span>Chef: {getChefServiceName(service.chefService)}</span>
                          </div>
            ...
            {ingenieurs.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {ingenieurs.map((ingenieur, idx) => (
                      <div key={ingenieur._id || ingenieur.id || `${ingenieur.email}-${idx}`} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                        <div className="flex items-center space-x-3 mb-3">
                          <div className="h-10 w-10 rounded-full bg-ocp-primary flex items-center justify-center">
                      <span className="text-white font-medium">
                        {(ingenieur.firstName?.[0] || ingenieur.name?.[0] || ingenieur.email[0]).toUpperCase()}
                        {(ingenieur.lastName?.[0] || '').toUpperCase()}
                      </span>
                          </div>
                          <div>
                            <h3 className="font-medium text-gray-900">
                              {getUserFullName(ingenieur)}
                            </h3>
