...
interface UserListSecteurProps {
  onEditUser: (user: User) => void;
  onViewUser: (user: User) => void;
  secteurId: string; // Required for chef secteur
}

export const UserListSecteur: React.FC<UserListSecteurProps> = ({
  onEditUser,
  onViewUser,
  secteurId
}) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Get all users in the secteur
      const response = await apiService.getUsersBySecteur(secteurId);
  ...
  const canManageUsers = currentUser?.role === 'chef_secteur';
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                disabled
              >
                <option value="all">Tous les utilisateurs du secteur</option>
              </select>
            </div>
          </div>
        </Card.Body>
        ...
        <Card.Body>
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">Aucun utilisateur trouvé dans ce secteur</p>
            </div>
          ) : (
      ...
      <div className="text-sm text-gray-700">
        Affichage de {filteredUsers.length} utilisateur(s) dans ce secteur
      </div>
    </div>
  );
};
